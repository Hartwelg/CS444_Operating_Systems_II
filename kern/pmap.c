...
mem_init_mp(void)
{
	...
	for(i = 0; i < NCPU; i++)
	{
		...
		// stackSpot = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
		// boot_map_region(kern_pgdir, stackSpot - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
	}
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(*PDEntry & PTE_P))
	{
		...
		*PDEntry = page2pa(newPageTable) | PTE_P | PTE_W | PTE_U;
		newPageTable->pp_ref++;
		pTable = (pte_t*) KADDR(page2pa((struct PageInfo*) newPageTable));
	}
	else
	{
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	physaddr_t pagePhysAddr = page2pa(pp);
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if (pageTableEntry)
	{
		...
		// physaddr_t physical = (struct PageInfo*) pa2page(PTE_ADDR(*pageTableEntry));
	}
	return NULL;
}
